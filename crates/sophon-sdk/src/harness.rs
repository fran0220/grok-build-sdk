...
impl HarnessDigest {
    fn for_content(content: &HarnessContent) -> Self {
        ...
        digest.update(b"sophon-sdk.harness-snapshot.v1\0");
...
impl TurnBindingReceipt {
    ...
    fn compute_binding_id(&self) -> Result<String, HarnessError> {
        ...
        let bytes = serde_json::to_vec(&value)
            .map_err(|error| HarnessError::Serialization(error.to_string()))?;
        let mut digest = Sha256::new();
        digest.update(b"sophon-sdk.turn-binding.v1\0");
        digest.update(bytes);
...
impl TurnBindingRecord {
    pub(crate) fn complete(
        ...
        let event_range_digest = digest_json(b"sophon-sdk.turn-binding-event-range.v1\0", events)?;
        let terminal_event_digest = digest_json(
            b"sophon-sdk.turn-binding-terminal-event.v1\0",
    ...
    fn compute_record_id(&self) -> Result<String, HarnessError> {
        digest_json(
            b"sophon-sdk.turn-binding-record.v1\0",

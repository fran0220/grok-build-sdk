...
fn run(
    ...
    bounds: KernelExecutionBounds,
    now_ms: u64,
) -> sophon_sdk::KernelExecutionReceipt {
...
impl KernelRuntime for OptimisticKernelRuntime {
    ...
    fn wait(
        &self,
        key: &KernelExecutionKey,
    ) -> Result<sophon_sdk::KernelExecutionReceipt, KernelError> {
        self.0.wait(key)
    }
    ...
    fn checkpoint(
        ...
        sink: &Arc<dyn ProgramOutputSink>,
        now_ms: u64,
    ) -> Result<sophon_sdk::KernelCheckpointRef, KernelError> {
    ...
    fn restore(
        &self,
        session: &KernelSessionId,
        checkpoint: &sophon_sdk::KernelCheckpointRef,
        spec: &KernelSpec,
        now_ms: u64,
    ...
    fn inspect_execution(
        &self,
        key: &KernelExecutionKey,
    ) -> Result<Option<sophon_sdk::KernelExecutionStatus>, KernelError> {

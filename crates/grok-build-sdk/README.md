...
| Host-owned native Session state | `RuntimeBuilder::session_state_store`, `SessionStateStore`, chunked `SessionObject`s, CAS `SessionManifest`, `LocalSessionStateStore` | Complete. With a Host store installed it is the sole authority for transcript/history, rewind state, and compaction checkpoints; its Session lease also fences create/load/resume/delete across Runtime instances. Covered JSONL files are neither read nor projected. Without injection the legacy JSONL backend remains available. |

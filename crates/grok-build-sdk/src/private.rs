...
impl StartupReporter {
    fn new(events: std::sync::mpsc::Sender<LifecycleEvent>) -> Self {
        Self {
            events: Some(events),
        }
    }

    fn report(mut self, result: Result<RuntimeCapabilities, Error>) {
        ...
        + 'static,
{
    ...
    std::thread::Builder::new()
        .name("grok-build-sdk-lifecycle".into())
...
mod tests {
    ...
    fn worker_panic_before_readiness_reports_startup_failure_and_is_joined() {
        ...
        let lifecycle =
            spawn_worker_lifecycle(commands, startup_tx, completion_tx, move |events| {
    ...
    fn executor_teardown_cancels_pending_startup_and_owned_worker_joins() {
        ...
        let lifecycle =
            spawn_worker_lifecycle(commands, startup_tx, completion_tx, move |_events| {
    ...
    async fn startup_readiness_racing_waiter_cancellation_retains_worker_ownership() {
        ...
        let lifecycle =
            spawn_worker_lifecycle(commands, startup_tx, completion_tx, move |events| {
                let worker =
                    lifecycle_test_worker(command_rx, shutdown_tx, release_rx, worker_joined);
                events

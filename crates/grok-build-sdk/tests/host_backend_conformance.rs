...
fn a_backend_that_drops_commits_fails_the_run_store_conformance() {
    ...
    impl RunStore for Amnesiac {
        fn load(
            &self,
            _: &grok_build_sdk::run::RunId,
        ) -> Result<Option<grok_build_sdk::run::RunEnvelope>, grok_build_sdk::run::RunError>
        {
            Ok(None)
        }
        fn list(
            &self,
        ) -> Result<Vec<grok_build_sdk::run::RunEnvelope>, grok_build_sdk::run::RunError> {
            Ok(Vec::new())
        }
        fn commit(
            &self,
            _: grok_build_sdk::run::StoreCommit,
        ) -> Result<grok_build_sdk::run::StoreCommitResult, grok_build_sdk::run::RunError> {
            Ok(grok_build_sdk::run::StoreCommitResult::Applied)
        }
    }

...
impl McpElicitationUi for ProductUi {
    ...
    ) -> Result<grok_build_sdk::mcp_model::ElicitResult, McpHostServiceError> {
        self.requests.lock().unwrap().push(request);
        Ok(grok_build_sdk::mcp_model::ElicitResult::new(
            grok_build_sdk::mcp_model::ElicitationAction::Accept,
        )
...
impl McpElicitationService for GenericElicitation {
    async fn create_elicitation(
        ...
        _request: grok_build_sdk::mcp_model::ElicitRequestParams,
    ) -> Result<grok_build_sdk::mcp_model::ElicitResult, McpHostServiceError> {
        Ok(grok_build_sdk::mcp_model::ElicitResult::new(
            grok_build_sdk::mcp_model::ElicitationAction::Decline,
        ))
    }
}
...
fn durable_task_identity_decode_is_bounded_and_fail_closed() {
    ...
    let invalid = McpTaskIdentity::new(
        grok_build_sdk::SessionId::from_stored("session-1"),
        "provider",
    ...
    assert!(
        McpTaskHandle {
            session_id: grok_build_sdk::SessionId::from_stored("session-1"),

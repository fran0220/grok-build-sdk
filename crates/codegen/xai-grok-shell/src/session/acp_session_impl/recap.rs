...
impl SessionActor {
    ...
    /// entirely** instead of fired doomed. The selected catalog entry supplies
    /// its own endpoint, credential, headers, backend, and wire model. The
    /// session model is never used: a per-turn background call must stay on
    /// the small model.
